...
class FKModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fk/ablation/figure_N/don_train_girloss_full_'+str(self.N)+'.png')
        plt.clf()
        plt.plot(ep, self.don_metrics.mean(-1), label='DeepONet')
        plt.fill_between(ep, self.don_metrics.mean(-1) - self.don_metrics.std(-1), self.don_metrics.mean(-1) + self.don_metrics.std(-1), alpha=0.2)
        plt.ylabel('Relative Error')
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fk/ablation/figure_N/don_train_girloss_don_'+str(self.N)+'.png')
        plt.clf()

        torch.save(self.branch.state_dict(), '/scratch/xx84/girsanov/fk/ablation/trained_model_N/branch_'+str(self.N)+'.pt')
        torch.save(self.trunk.state_dict(), '/scratch/xx84/girsanov/fk/ablation/trained_model_N/trunk_'+str(self.N)+'.pt')
