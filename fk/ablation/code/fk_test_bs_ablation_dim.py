...
def terminal(dim, x, k):
    pxt = torch.max(x-k, dim=-1)[0]
    pxt[pxt<0] = 0
    return pxt
...
class FKModule(pl.LightningModule):
    def __init__(self, N = 2000, lr = 1e-3, X = 1., T = 0.1, dim = 2, batch_size = 100, num_time = 100, n_batch_val=100):
        ...
        self.sensors = terminal(self.dim, (torch.linspace(0., 1., self.m).unsqueeze(-1).repeat(1,self.dim) * self.X).to(device), self.k)
    ...
    def loss(self, xt, coef):
        ...
        #p0Bx_gir = terminal(dim, Bx_gir, self.k).squeeze()
        #start = time.time()
        #muBx_gir = drift(Bx_gir, coef, self.r)
        ...
        u_don = (branchs * trunks).sum(1).reshape(u_cnn.shape)
        end = time.time()
        time_don = (end - start)
        return u_em, u_cnn, u_don, u_gt, time_em, time_cnn, time_don
    ...
    def training_step(self, batch, batch_idx):
        ...
        u_em, u_cnn, u_don, u_gt, time_em, time_cnn, time_don = self.loss(xt, coef=torch.rand(1,1,1,3).to(device))
        #plt.subplot(3,2,1)
        ...
        loss = F.l1_loss(u_cnn, u_em)#/(torch.abs(u_gir).mean())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print('Validation: {:.4f}, {:.4f}'.format(loss_cnn, loss_don))
        ...
        if not loss_cnn.isnan():
            if not loss_cnn.isinf():
                ...
                #self.cnn_metrics[self.cnn_metrics == np.inf] = 0
                #self.cnn_metrics[self.cnn_metrics == np.nan] = 0
                #self.gir_metrics[self.gir_metrics == np.inf] = 0
                #self.gir_metrics[self.gir_metrics == np.nan] = 0
                #self.don_metrics[self.don_metrics == np.inf] = 0
                #self.don_metrics[self.don_metrics == np.nan] = 0
                #self.em_metrics[self.em_metrics == np.inf] = 0
                #self.em_metrics[self.em_metrics == np.nan] = 0
                ...
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_cnn_loss.npy', 'wb') as f:
                    np.save(f, np.array(self.cnn_metrics))
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_em_loss.npy', 'wb') as f:
                    np.save(f, np.array(self.em_metrics))
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_don_loss.npy', 'wb') as f:
                    np.save(f, np.array(self.don_metrics))
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_cnn_time.npy', 'wb') as f:
                    np.save(f, np.array(self.cnn_comp_time))
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_em_time.npy', 'wb') as f:
                    np.save(f, np.array(self.em_comp_time))
                with open('/scratch/xx84/girsanov/fk/bs/result/'+str(dim)+'_'+str(self.num_time)+'_bs_don_time.npy', 'wb') as f:
                    np.save(f, np.array(self.don_comp_time))
...
if __name__ == '__main__':
    ...
    #ep = torch.arange(18)
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_loss_mean.npy', 'wb') as f:
            np.save(f, np.array(cnn_loss_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_loss_min.npy', 'wb') as f:
            np.save(f, np.array(cnn_loss_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_loss_max.npy', 'wb') as f:
            np.save(f, np.array(cnn_loss_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_loss_var.npy', 'wb') as f:
            np.save(f, np.array(cnn_loss_var))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_loss_mean.npy', 'wb') as f:
        #    np.save(f, np.array(gir_loss_mean))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_loss_min.npy', 'wb') as f:
        #    np.save(f, np.array(gir_loss_min))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_loss_max.npy', 'wb') as f:
        #    np.save(f, np.array(gir_loss_max))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_loss_var.npy', 'wb') as f:
        #    np.save(f, np.array(gir_loss_var))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_loss_mean.npy', 'wb') as f:
            np.save(f, np.array(don_loss_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_loss_min.npy', 'wb') as f:
            np.save(f, np.array(don_loss_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_loss_max.npy', 'wb') as f:
            np.save(f, np.array(don_loss_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_loss_var.npy', 'wb') as f:
            np.save(f, np.array(don_loss_var))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_loss_mean.npy', 'wb') as f:
            np.save(f, np.array(em_loss_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_loss_min.npy', 'wb') as f:
            np.save(f, np.array(em_loss_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_loss_max.npy', 'wb') as f:
            np.save(f, np.array(em_loss_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_loss_var.npy', 'wb') as f:
            np.save(f, np.array(em_loss_var))
        
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_time_mean.npy', 'wb') as f:
            np.save(f, np.array(cnn_time_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_time_min.npy', 'wb') as f:
            np.save(f, np.array(cnn_time_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_time_max.npy', 'wb') as f:
            np.save(f, np.array(cnn_time_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_cnn_time_var.npy', 'wb') as f:
            np.save(f, np.array(cnn_time_var))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_time_mean.npy', 'wb') as f:
        #    np.save(f, np.array(gir_time_mean))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_time_min.npy', 'wb') as f:
        #    np.save(f, np.array(gir_time_min))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_time_max.npy', 'wb') as f:
        #    np.save(f, np.array(gir_time_max))
        #with open('/scratch/xx84/girsanov/fk/ablation/result/dim_ou_gir_time_var.npy', 'wb') as f:
        #    np.save(f, np.array(gir_time_var))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_time_mean.npy', 'wb') as f:
            np.save(f, np.array(don_time_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_time_min.npy', 'wb') as f:
            np.save(f, np.array(don_time_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_time_max.npy', 'wb') as f:
            np.save(f, np.array(don_time_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_don_time_var.npy', 'wb') as f:
            np.save(f, np.array(don_time_var))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_time_mean.npy', 'wb') as f:
            np.save(f, np.array(em_time_mean))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_time_min.npy', 'wb') as f:
            np.save(f, np.array(em_time_min))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_time_max.npy', 'wb') as f:
            np.save(f, np.array(em_time_max))
        with open('/scratch/xx84/girsanov/fk/ablation/result/dim_bs_em_time_var.npy', 'wb') as f:
