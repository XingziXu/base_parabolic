...
class FKModule(pl.LightningModule):
    def __init__(self, N = 1000, lr = 1e-30, X = 1., num_time = 50, T = 0.1, dim = 2, batch_size = 100):
        super().__init__()
        ...
        #torch.save(self.sequence.state_dict(), '/scratch/xx84/girsanov/pde_rnn/cnn_2d.pt')
        self.sequence.load_state_dict(torch.load('/scratch/xx84/girsanov/pde_rnn/cnn_2d1.pt'))
...
if __name__ == '__main__':
    ...
    plt.plot(Ts[1:], val_rnn[1:], label='cnn')
    plt.plot(Ts[1:], val_gir[1:], label='girsanov')
    plt.xlabel('Time')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('/scratch/xx84/girsanov/pde_rnn/loss_time_2d1.png')
    plt.clf()
    plt.plot(Ts[1:], time_em[1:], label='em')
    plt.plot(Ts[1:], time_gir[1:], label='girsanov')
    plt.plot(Ts[1:], time_rnn[1:], label='cnn')
    plt.xlabel('Time')
    plt.ylabel('Computational Time')
    plt.legend()
    plt.savefig('/scratch/xx84/girsanov/pde_rnn/comptime_time_2d1.png')
    plt.clf()
