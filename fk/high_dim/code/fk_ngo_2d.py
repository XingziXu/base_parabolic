...
class FKModule(pl.LightningModule):
    def __init__(self, N = 2000, lr = 1e-3, X = 1., T = 0.05, dim = 2, batch_size = 100, num_time = 100, n_batch_val=100):
        ...
        self.dB = np.sqrt(self.dt.item()) * np.random.randn(self.t.shape[0], self.N, 50, self.dim)
        self.dB[0,:,:,:] = 0 
    ...
    def loss(self, xt, coef):
        ...
        #ts = xt[:,-1]
        indices = torch.randperm(len(xs))[:50]
        xs = xs[indices,:]
        coef = coef
        ...
        start = time.time()
        x = torch.zeros(self.num_time, self.N, 50, self.dim).to(device)
        ...
        start = time.time()
        input = torch.zeros(self.num_time, self.N, 50, self.dim * 2 + 1).to(device)
    ...
    def training_step(self, batch, batch_idx):
        # REQUIRED
        xt = batch.to(device)
        u_em, u_gir, u_fno, time_em, time_gir, time_cnn = self.loss(xt, coef=torch.rand(1,1,1,3).to(device))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with open('/scratch/xx84/girsanov/fk/high_dim/figure/ngo_comp_time_'+str(self.dim)+'.npy', 'wb') as f:
        ...
        plt.plot(ep, self.cnn_metrics.mean(-1), label='NGO')
