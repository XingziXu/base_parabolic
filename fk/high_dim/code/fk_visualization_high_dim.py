...
ax.plot(ep, np.array(gir_mean_t), label='Girsanov',color='goldenrod')
ax.fill_between(ep, np.array(gir_mean_t-gir_var_t), np.array(gir_mean_t+gir_var_t), alpha=0.2,color='goldenrod')
ax.grid()
...
plt.savefig('/scratch/xx84/girsanov/fk/'+name+'/figure/'+str(dim)+'_time_fk.pdf')
plt.clf()
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.savefig('/scratch/xx84/girsanov/fk/'+name+'/figure/'+str(dim)+'_loss_fk.pdf')
plt.clf()