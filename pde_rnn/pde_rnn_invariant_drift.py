...
class FKModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        # REQUIRED
        xtu = batch.to(device)
        loss_total = self.loss(xtu, self.gwt, coef = torch.Tensor([[0.0087, 0.1206, 0.0780, 0.1100, 0.0175, 0.2102]]))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xtu = batch.to(device)
        loss_total = self.loss(xtu, self.gwt_val, coef = torch.Tensor([[0.0155, 0.1404, 0.2021, 0.0994, 0.2351, 0.1689]]))
        #tensorboard_logs = {'train_loss': loss_prior}
        self.log('val_loss', loss_total)
        print(loss_total)
        """
...
if __name__ == '__main__':
    ...
    coef = torch.Tensor([[0.0087, 0.1206, 0.0780, 0.1100, 0.0175, 0.2102]])
    coef_val = torch.Tensor([[0.0155, 0.1404, 0.2021, 0.0994, 0.2351, 0.1689]])
