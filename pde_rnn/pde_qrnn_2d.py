...
class qrnn(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_outputs):
        ...
        self.qrnn = QRNN(input_size, hidden_size, num_layers=2, dropout=0.)
...
class FKModule(pl.LightningModule):
    def __init__(self, N = 1000, lr = 1e-3, X = 1., T = 0.1, dim = 2, batch_size = 100, num_time = 100):
        ...
        self.sequence = qrnn(input_size, hidden_size, num_layers, num_outputs)
    ...
    def loss(self, xt, coef):
        ...
        input_reshaped = input.reshape(input.shape[0], input.shape[1]*input.shape[2], input.shape[3])
