...
class PDETask:
    def __init__(self, num_data=int(10000), coef = torch.rand(1, 6) * 0.3):
        self.num_data = num_data
        self.num_path = 2000 # simulate 500 runs of brownian motion
        self.num_time = 50 # simulate brownian motion with 500 time steps
        self.num_pos = 200 # simulate the space in 100 time steps
        self.t_init = 0
        self.t_end = 0.05 # define the ending time
        self.x_init = 0
...
class FKModule(pl.LightningModule):
    ...
    def loss(self, xtu, gwt, coef):
        ...
        for i in range(len(xtu)):
            ...
            B_current = self.drift(x=torch.tensor(self.B[:,x_idx[i],:t_idx[i]]).unsqueeze(2),t=t_idx[i]*self.dt, coef=coef).unsqueeze(2)
            ...
            # form the input
            input_current = torch.cat((dB_current, B_current, ds_current),dim=2).to(device)
    ...
    def training_step(self, batch, batch_idx):
        # REQUIRED
        xtu = batch.to(device)
        loss_total = self.loss(xtu, self.gwt, coef = torch.Tensor([[0.0087, 0.1206, 0.0780, 0.1100, 0.0175, 0.2102]])*3)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        xtu = batch.to(device)
        loss_total = self.loss(xtu, self.gwt_val, coef = torch.Tensor([[0.0155, 0.1404, 0.2021, 0.0994, 0.2351, 0.1689]])*3)
        #tensorboard_logs = {'train_loss': loss_prior}
        self.log('val_loss', loss_total)
        print(loss_total)
        """
...
if __name__ == '__main__':
    ...
    coef = torch.Tensor([[0.0087, 0.1206, 0.0780, 0.1100, 0.0175, 0.2102]])*3
    coef_val = torch.Tensor([[0.0155, 0.1404, 0.2021, 0.0994, 0.2351, 0.1689]])*3
    task = PDETask(coef=coef)
    task_val = PDETask_val(coef=coef_val)
