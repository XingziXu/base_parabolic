...
plt.savefig('/scratch/xx84/girsanov/generative_modeling/nf/initial.png')
plt.clf()
...
p1 = MultivariateNormal(torch.ones(dim) * 0.2, torch.eye(dim))
p2 = MultivariateNormal(torch.ones(dim) * 0.4, torch.eye(dim))
p3 = MultivariateNormal(torch.ones(dim) * 0.6, torch.eye(dim))
p4 = MultivariateNormal(torch.ones(dim) * 0.8, torch.eye(dim))
p5 = MultivariateNormal(torch.ones(dim) * 1.0, torch.eye(dim))
p6 = MultivariateNormal(torch.ones(dim) * 1.2, torch.eye(dim))
p7 = MultivariateNormal(torch.ones(dim) * 1.4, torch.eye(dim))
p8 = MultivariateNormal(torch.ones(dim) * 1.6, torch.eye(dim))
p9 = MultivariateNormal(torch.ones(dim) * 1.8, torch.eye(dim))
...
for it in tqdm(range(max_iter)):
    ...
    if (it + 1) % show_iter == 0:
        ...
        plt.pcolormesh(xx, yy, prob.data.numpy())
        plt.gca().set_aspect('equal', 'box')
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/nf/posterior.png')
        plt.clf()
...
plt.legend()
plt.savefig('/scratch/xx84/girsanov/generative_modeling/nf/loss.png')
...
plt.pcolormesh(xx, yy, prob.data.numpy())
plt.gca().set_aspect('equal', 'box')
plt.savefig('/scratch/xx84/girsanov/generative_modeling/nf/posterior_final.png')
torch.save(model.state_dict(), '/scratch/xx84/girsanov/generative_modeling/nf/nf_prior.pt')