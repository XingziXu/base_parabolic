...
class FKModule(pl.LightningModule):
    def __init__(self, batch_size):
        ...
        self.mu0 = SMLP(hyper_param['dim'], 200, 3, hyper_param['dim'], nn.Tanh())
        self.mu0.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/dim_100/mu_20.pt'))
    ...
    def loss(self, batch):
        ...
        for i in range(self.num_steps-2):
            ...
            mu_current = (self.mu(y_current) + self.mu0(y_current)).to(device)
    ...
    def sample(self, num_sample):
        with torch.no_grad():
            ...
            for i in range(self.num_steps_val-2):
                ...
                mu_current = (self.mu(x_current) + self.mu0(x_current)).to(device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        plt.clf()
        torch.save(self.mu.state_dict(), '/scratch/xx84/girsanov/generative_modeling/fk/trained_model/dim_100/mu_current.pt')
        """
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW([{'params': self.mu.parameters()}], lr=8e-4)
...
if __name__ == '__main__':
    ...
    for i in range(100):
        p_i = MultivariateNormal(torch.zeros(hyper_param['dim']) + 0.2 * i * torch.ones(hyper_param['dim']), torch.eye(hyper_param['dim']))
        trainset = p_i.sample([1000])
        ...
        model = FKModule(batch_size = batch_size)
        trainer = pl.Trainer(max_epochs=3,gpus=1)
        trainer.fit(model, train_loader, test_loader)
        
        testing_mag_data = 10 * torch.rand(size=(10000,hyper_param['dim']))
        with torch.no_grad():
            mu = SMLP(hyper_param['dim'], 200, 3, hyper_param['dim'], nn.Tanh())
            mu.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/dim_100/mu_current.pt'))
            mu0 = SMLP(hyper_param['dim'], 200, 3, hyper_param['dim'], nn.Tanh())
            mu0.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/dim_100/mu_20.pt'))
            
            mu_val = mu(testing_mag_data)
            mu0_val = mu0(testing_mag_data)
            diff = mu0_val - mu_val
        diff = abs(diff).mean()
        mag.append(diff.item())
        ...
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/100dgaussian_bpd_mu_0.npy', 'wb') as f:
            np.save(f, val_bpd)
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/100dgaussian_loss_mu_0.npy', 'wb') as f:
            np.save(f, loss)
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/100dgaussian_mag_mu_0.npy', 'wb') as f:
            np.save(f, mag)
        fig = plt.figure()
        ...
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/fk/figure/bpd_toy_fokker_planck_100d_mu.png')
        
        fig = plt.figure()
        ax0 = fig.add_subplot(111)
        ax0.scatter(mag, val_bpd)
        ax0.set_ylabel('ELBO Loss')
        #ax1 = fig.add_subplot(212)
        #ax1.plot(loss[2:], loss_time[2:])
        #ax1.scatter(loss[0:2], loss_time[0:2])
        #ax1.set_ylabel('integration time')
        ax0.set_xlabel('Change magnitude')
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/fk/figure/mag_toy_fokker_planck_100d_mu.png')
        if i > 2:
