...
class FKModule(pl.LightningModule):
    def __init__(self, batch_size):
        ...
        self.mu = SMLP(2, 200, 3, 2, nn.Tanh())
        self.mu0 = SMLP(2, 200, 3, 2, nn.Tanh())
        self.mu0.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/mu_20.pt'))
        ...
        self.dW = torch.randn(self.N, self.batch_size, self.num_steps-2, 2).to(device)
    ...
    def sample(self, num_sample):
        with torch.no_grad():
            ...
            self.dt_val = self.t_val[1]-self.t_val[0]
            self.sdt_val = torch.sqrt(self.dt_val).to(device)
            dW = self.sdt_val * torch.randn(num_sample, self.num_steps_val-2, 2).to(device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        plt.clf()
        torch.save(self.mu.state_dict(), '/scratch/xx84/girsanov/generative_modeling/fk/trained_model/mu_current.pt')
        """
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        testing_mag_data = 10 * torch.rand(size=(10000,2))
        with torch.no_grad():
            mu = SMLP(2, 200, 3, 2, nn.Tanh())
            mu.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/mu_current.pt'))
            mu0 = SMLP(2, 200, 3, 2, nn.Tanh())
            mu0.load_state_dict(torch.load('/scratch/xx84/girsanov/generative_modeling/fk/trained_model/mu_20.pt'))
        ...
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/2dgaussian_bpd_mu_0.npy', 'wb') as f:
            np.save(f, val_bpd)
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/2dgaussian_loss_mu_0.npy', 'wb') as f:
            np.save(f, loss)
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/2dgaussian_mag_mu_0.npy', 'wb') as f:
            np.save(f, mag)
        fig = plt.figure()
        ...
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/fk/figure/bpd_toy_fokker_planck_2d_mu.png')
        ...
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/fk/figure/mag_toy_fokker_planck_2d_mu.png')
        if i > 2:
