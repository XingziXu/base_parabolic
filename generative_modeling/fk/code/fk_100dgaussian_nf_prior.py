...
class FKModule(pl.LightningModule):
    def __init__(self, batch_size, dim, n_batch_val):
        super().__init__()
        self.dim = dim
        ...
        base = nf.distributions.base.DiagGaussian(self.dim)
        ...
        for i in range(num_layers):
            ...
            param_map = SMLP_nf(int(self.dim/2), 128, 3, self.dim, nn.Tanh())
            # Add flow layer
    ...
    def loss(self, batch):
        ...
        for i in range(self.num_steps-2):
            ...
            div = div - self.dt * trace_grad(mu_current, y_current, N=1).squeeze()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #self.log('val_loss', torch.mean(self.metrics))
        batch = batch.cpu().numpy()
        #self.metrics[self.current_epoch, batch_idx] = val_loss.item()
...
if __name__ == '__main__':
    ...
    for i in range(num_layers):
        ...
        # Last layer is initialized by zeros making training more stable
        #param_map = nf.nets.MLP([1, 64, 64, 2], init_zeros=True)
        param_map = SMLP_nf(int(dim/2), 128, 3, dim, nn.Tanh())
        # Add flow layer
        flows.append(nf.flows.AffineCouplingBlock(param_map))
        # Swap dimensions
        flows.append(nf.flows.Permute(2, mode='swap'))
    ...
    for i in range(100):
        ...
        batch_size = 150
        num_samples = 150
        trainset = p_i.sample([1500])
        testset = p_i.sample([150])
        ...
        n_batch_val = int(2000 / batch_size)-1
        model = FKModule(batch_size, dim, n_batch_val)
        ...
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/100dgaussian_bpd_nf_0.npy', 'wb') as f:
            np.save(f, val_bpd)
        with open('/scratch/xx84/girsanov/generative_modeling/fk/result/100dgaussian_loss_nf_0.npy', 'wb') as f:
            np.save(f, loss)
        fig = plt.figure()
        ...
        plt.savefig('/scratch/xx84/girsanov/generative_modeling/fk/figure/bpd_toy_fokker_planck_nf_100d.png')
