...
class FKModule(pl.LightningModule):
    def __init__(self):
        ...
        self.mu = SMLP(2, 256, 3, 2, nn.Tanh())
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=8e-4)
...
if __name__ == '__main__':
    ...
    #p0 = MultivariateNormal(torch.zeros(2), torch.eye(2))
    #d0 = p0.sample([1000]).data.numpy()
    ...
    sampler_moon = Moons()
    d_moon_t = sampler_moon.sample(30000).cpu().data.numpy()
    train_loader_moon = torch.utils.data.DataLoader(d_moon_t, batch_size = 300, shuffle=True, num_workers = 1)
    d_moon_v = sampler_moon.sample(1000).cpu().data.numpy()
    val_loader_moon = torch.utils.data.DataLoader(d_moon_v, batch_size = 1000, shuffle=False)
    #loss.append(Loss(torch.tensor(d0).type(torch.FloatTensor),torch.tensor(d_moon_v).type(torch.FloatTensor)).item())
    model = FKModule()
    trainer = pl.Trainer(max_epochs=100,gpus=1)
    trainer.fit(model, train_loader_moon, val_loader_moon)
    #train_loss.append(trainer.logged_metrics['train_loss'])
    #loss_time.append(trainer.logged_metrics['loss_time'])