...
class FKModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fbsde/ngo/figure/ngo_train_girloss_full.png')
        plt.clf()
        plt.plot(ep, self.metrics.mean(-1), label='CNN')
        plt.fill_between(ep, self.metrics.mean(-1) - self.metrics.std(-1), self.metrics.mean(-1) + self.metrics.std(-1), alpha=0.2)
        plt.ylabel('Relative Error')
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fbsde/ngo/figure/ngo_train_girloss_ngo.png')
        plt.clf()
