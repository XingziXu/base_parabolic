...
class FKModule(pl.LightningModule):
    ...
    def loss(self, xt, coef, coef1, em = False):
        ...
        start = time.time()
        xs = torch.linspace(0., 1., steps=self.batch_size).unsqueeze(-1).to(device)
        sigmas = np.sqrt(2)
        ...
        if em:
            ...
            with open('/scratch/xx84/girsanov/fbsde/hjb/result/hjb_cnn_1d_vis.npy', 'wb') as f:
                np.save(f, np.array(v_cnn.detach().cpu()))
            with open('/scratch/xx84/girsanov/fbsde/hjb/result/hjb_gt_1d_vis.npy', 'wb') as f:
                np.save(f, np.array(v_gt.detach().cpu()))
            return v_cnn, v_em, v_gt, time_cnn, time_em
        return v_cnn, v_gt, time_cnn

    def training_step(self, batch, batch_idx):
        # REQUIRED
        xt = batch.to(device)
        v_cnn, v_em, v_gt, time_cnn, time_em = self.loss(xt, coef=torch.rand(1,1,1,3).to(device), coef1=torch.rand(1,1,1,3).to(device), em=True)
        
        #v_cnn = v_cnn[~torch.any(v_cnn.isnan(),dim=1)]
        #v_cnn = v_cnn[~torch.any(v_cnn.isnan(),dim=1)]
        loss = F.l1_loss(v_cnn, v_em)#+F.l1_loss(zi_cnn, zi_gir)#+F.l1_loss(zi_cnn, zi_em)+F.l1_loss(v_cnn, v_em)#/(torch.abs(u_gir).mean())
