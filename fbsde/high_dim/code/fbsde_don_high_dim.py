...
class FKModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not loss_don.isnan():
            ...
            self.don_comp_time[self.current_epoch, batch_idx] = time_cnn
        ...
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fbsde/high_dim/figure/don_train_girloss_full_'+str(self.dim)+'.png')
        plt.clf()
        plt.plot(ep, self.don_metrics.mean(-1), label='DeepONet')
        plt.fill_between(ep, self.don_metrics.mean(-1) - self.don_metrics.std(-1), self.don_metrics.mean(-1) + self.don_metrics.std(-1), alpha=0.2)
        plt.ylabel('Relative Error')
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fbsde/high_dim/figure/don_train_girloss_don_'+str(self.dim)+'.png')
        plt.clf()
        ...
        plt.plot(ep, self.don_comp_time.mean(-1), label='DeepONetN')
        plt.fill_between(ep, self.don_comp_time.mean(-1) - self.don_comp_time.std(-1), self.don_comp_time.mean(-1) + self.don_comp_time.std(-1), alpha=0.2)
        plt.ylabel('Computation Time')
        plt.xlabel('Epochs')
        plt.legend()
        plt.savefig('/scratch/xx84/girsanov/fbsde/high_dim/figure/don_train_comptime_'+str(self.dim)+'.png')
        plt.clf()
        torch.save(self.branch.state_dict(), '/scratch/xx84/girsanov/fbsde/high_dim/trained_model/branch_'+str(self.dim)+'.pt')
        torch.save(self.trunk.state_dict(), '/scratch/xx84/girsanov/fbsde/high_dim/trained_model/trunk_'+str(self.dim)+'.pt')
